...
void ImprovedBranchBoundGraphBipartitioningDeltaContribNVFREELogic<Pheet, SubProblem>::update(uint8_t set, size_t pos) {
	...
	while(current_bit != sub_problem->sets[2].size()) {
		...
		// compute the free degree (TBD incrementally) DONE
		/* 
		freedeg[v] = 0;
		for (size_t j=0; j<sub_problem->graph[v].num_edges; j++) {
		  if (sub_problem->sets[2].test(sub_problem->graph[v].edges[j].target)) freedeg[v]++;
		}
		if (freedeg[v]!=free_edges[v]) std::cout <<'-';
		*/
