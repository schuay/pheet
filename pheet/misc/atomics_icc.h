...
template <typename T>
inline T* ptr_compare_and_swap(T** p, T* old_v, T* new_v) {
    return __sync_val_compare_and_swap(p, old_v, new_v);
}
template <typename T>
inline T* ptr_compare_and_swap(T** p, std::nullptr_t old_v, T* new_v) {
    return __sync_val_compare_and_swap(p, static_cast<T*>(old_v), new_v);
}
