/*
 * ArrayListStrategyTaskStorageIterator.h
 *
...
#ifndef LINKEDARRAYLISTSTRATEGYTASKSTORAGEITERATOR_H_
#define LINKEDARRAYLISTSTRATEGYTASKSTORAGEITERATOR_H_
...
class ArrayListStrategyTaskStorageIterator {
public:
	typedef ArrayListStrategyTaskStorageIterator<Storage> ThisType;

	ArrayListStrategyTaskStorageIterator();
	ArrayListStrategyTaskStorageIterator(size_t index);
	ArrayListStrategyTaskStorageIterator(ThisType const& other);
	~ArrayListStrategyTaskStorageIterator();
 ...
 */
template <class Storage>
ArrayListStrategyTaskStorageIterator<Storage>::ArrayListStrategyTaskStorageIterator()
: control_block(NULL), control_block_item_index(Storage::block_size), control_block_item(NULL), ffwd(false) {

}

template <class Storage>
ArrayListStrategyTaskStorageIterator<Storage>::ArrayListStrategyTaskStorageIterator(size_t index)
: index(index), control_block(NULL), control_block_item_index(Storage::block_size), control_block_item(NULL), ffwd(false) {

}

template <class Storage>
inline ArrayListStrategyTaskStorageIterator<Storage>::ArrayListStrategyTaskStorageIterator(ThisType const& other)
...
template <class Storage>
inline size_t ArrayListStrategyTaskStorageIterator<Storage>::get_index() const {
	return index;
}

template <class Storage>
inline ptrdiff_t ArrayListStrategyTaskStorageIterator<Storage>::operator-(ThisType const& other) const {
	return index - other.index;
}

template <class Storage>
inline ArrayListStrategyTaskStorageIterator<Storage>& ArrayListStrategyTaskStorageIterator<Storage>::operator++() // prefix
{
	if(ffwd) {
...
template <class Storage>
ArrayListStrategyTaskStorageIterator<Storage>& ArrayListStrategyTaskStorageIterator<Storage>::operator=(ThisType const& other) {
	index = other.index;
...
template <class Storage>
inline bool ArrayListStrategyTaskStorageIterator<Storage>::operator==(ThisType const& other) const {
	return index == other.index;
}
...
template <class Storage>
typename Storage::Item* ArrayListStrategyTaskStorageIterator<Storage>::dereference(Storage* storage) {
	size_t block_index;
...
template <class Storage>
void ArrayListStrategyTaskStorageIterator<Storage>::deactivate() {
	if(control_block != NULL) {
		control_block->deregister_iterator();
	}
}
...
#endif /* LINKEDARRAYLISTSTRATEGYTASKSTORAGEITERATOR_H_ */
