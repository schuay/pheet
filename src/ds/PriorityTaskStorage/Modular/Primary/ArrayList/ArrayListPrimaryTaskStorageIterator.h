...
public:
	...
	size_t operator-(ThisType const& other) const;
	ThisType& operator++();
	ThisType& operator=(ThisType const& other);
	bool operator==(ThisType const& other) const;
	bool operator!=(ThisType const& other) const;

	Storage::Item* dereference(Storage::ControlBlock* current_control_block);
private:
	size_t index;
	Storage::ControlBlock* control_block;
	size_t control_block_item_index;
	Storage::ControlBlock::Item* control_block_item;
	bool ffwd;
};

template <class Storage>
ArrayListPrimaryTaskStorageIterator<Storage>::ArrayListPrimaryTaskStorageIterator(size_t index)
: index(index), control_block(NULL), control_block_item(NULL), block_item_index(Storage::block_size), ffwd(false) {

}

template <class Storage>
inline ArrayListPrimaryTaskStorageIterator<Storage>::ArrayListPrimaryTaskStorageIterator(ThisType const& other)
: index(other.index), control_block(NULL), control_block_item(NULL), block_item_index(Storage::block_size) {
...
template <class Storage>
Storage::Item* ArrayListPrimaryTaskStorageIterator<Storage>::dereference(Storage* storage) {
	size_t block_index;
