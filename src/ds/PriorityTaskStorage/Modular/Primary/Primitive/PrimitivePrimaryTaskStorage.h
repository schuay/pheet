...
struct PrimitiveTaskStorageItem {
	TT data;
	BaseStrategy* s;
	prio_t pop_prio;
	size_t index;
...
private:
	...
	CircularArray<PrimitiveTaskStorageItem<T> > data;
...
TT PrimitivePrimaryTaskStorage<TT, CircularArray>::take(iterator item) {
	...
	PrimitiveTaskStorageItem<T>& ptsi = data.get(item);
...
inline TT PrimitivePrimaryTaskStorage<TT, CircularArray>::peek() {
	...
	iterator best = i;
	PrimitiveTaskStorageItem<T> ret_item = data.get(i);
	prio_t best_prio = ret_item.pop_prio;
	++i;
	for(; i != end(); ++i) {
		if(!is_taken(i)) {
			PrimitiveTaskStorageItem<T> tmp_item = data.get(i);
